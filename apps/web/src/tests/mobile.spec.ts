...
test.describe('iPhone 12', () => {
  ...
  test('schedule page - filters toggle on mobile', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Recreational")')).toHaveClass(/bg-primary-500/);
  });
  ...
  test('map page - loads and displays markers', async ({ page }) => {
    ...
    if (markerCount > 0) {
      ...
      await expect(page.locator('text=/Showing.*pools with lane swim/')).toBeVisible();
    }
  });

  test('map page - facility details sidebar is responsive', async ({ page }) => {
    await page.goto('/map');
    ...
    if (markerCount > 0) {
      ...
      // Sidebar should be visible and take up most of the width on mobile
