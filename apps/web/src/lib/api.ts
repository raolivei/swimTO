...
export const scheduleApi = {
  getSchedule: async (filters?: ScheduleFilters): Promise<SessionWithFacility[]> => {
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
      return {
        ...
        details: `Timeout after ${(axios.defaults.timeout || 30000) / 1000} seconds`,
        suggestions: [
    ...
    if (error.response) {
      ...
      return {
        ...
        details: `HTTP ${status}: ${error.response.statusText || error.message}`,
        suggestions: [
          'Try refreshing the page',
